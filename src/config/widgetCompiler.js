...
class WidgetsCompilerPlugin {
  constructor(publicRuntimeConfig, webpackConfig, webpackInstance) {
    ...
    const modifiedPlugins = webpackConfig.plugins.filter(
    ...
    this.buildConfig = {
      ...
      optimization: {
        ...
        splitChunks: {
          cacheGroups: {
            widgets: {
              name: "jkdjnsjkahdyuiqwhduysgbfvjhkdbchjvbwdjhfgeuiwygfouywe",
              priority: 0,
              chunks: "async",
              enforce: true,
            },
            vendors: {
              name: "knfiewhbvdbsvmnxbcvweiphvweioghuyreobvfdbvfbnm",
              priority: 0,
              test: /[\\/]node_modules[\\/]/,
              chunks: "async",
  ...
  apply(compiler) {
    compiler.hooks.beforeRun.tapPromise("Widgets compiler", () =>
      this.compileWidgets()
    );
  }
